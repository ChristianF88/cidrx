...
// CollectCIDRsCoreParallel - Optimized non-recursive version for parallel processing
func (t *Trie) CollectCIDRsCoreParallel(node *TrieNode, prefix uint32, depth uint32, results *[]string,
	minClusterSize, minDepth, maxDepth uint32, threshold uint32, builderPool *sync.Pool) {
	...
	for len(stack) > 0 {
		...
		if item.depth == maxDepth {
			if item.node.Count >= minClusterSize {
				*results = append(*results, prefixToCIDR(item.prefix, item.depth, builderPool))
			}
			continue
		}
...
// CollectCIDRsCoreSequential - Sequential version for better understanding and smaller workloads
func (t *Trie) CollectCIDRsCoreSequential(node *TrieNode, prefix uint32, depth uint32, results *[]string, minClusterSize, minDepth, maxDepth uint32, threshold uint32, builderPool *sync.Pool) {
	// Check for nil node
	// and if the count is below the minimum cluster size
	if node == nil {
		return
	}
	...
	if depth == maxDepth {
		if node.Count >= minClusterSize {
			*results = append(*results, prefixToCIDR(prefix, depth, builderPool))
		}
		return
	}
	...
	if hasLeft && hasRight {
		...
		if leftCount <= rightCount {
			if leftCount >= minClusterSize {
				t.CollectCIDRsCoreSequential(node.Children[0], prefix, depth+1,
					results, minClusterSize, minDepth, maxDepth, threshold, builderPool)
			}
			if rightCount >= minClusterSize {
				t.CollectCIDRsCoreSequential(node.Children[1], prefix|(1<<(31-depth)), depth+1,
					results, minClusterSize, minDepth, maxDepth, threshold, builderPool)
			}
		} else {
			if rightCount >= minClusterSize {
	...
	// Recursively traverse the left and right children with early exit optimization
	if hasLeft && node.Children[0].Count >= minClusterSize {
		t.CollectCIDRsCoreSequential(node.Children[0], prefix, depth+1, results, minClusterSize, minDepth, maxDepth, threshold, builderPool)
	}
	if hasRight && node.Children[1].Count >= minClusterSize {
		t.CollectCIDRsCoreSequential(node.Children[1], prefix|(1<<(31-depth)), depth+1, results, minClusterSize, minDepth, maxDepth, threshold, builderPool)
	}
}
...
// prefixToCIDR converts a prefix and depth to CIDR string notation using string builder pool
// Legacy function for backward compatibility
func prefixToCIDR(prefix uint32, depth uint32, builderPool *sync.Pool) string {
	builder := pools.GetBuilderFromPool(builderPool)
	// Pre-allocate capacity for typical IPv4 CIDR string
	builder.Grow(18)

	// Format the IP address components
	builder.WriteString(strconv.Itoa(int((prefix >> 24) & 0xFF)))
	builder.WriteByte('.')
	builder.WriteString(strconv.Itoa(int((prefix >> 16) & 0xFF)))
	builder.WriteByte('.')
	builder.WriteString(strconv.Itoa(int((prefix >> 8) & 0xFF)))
	builder.WriteByte('.')
	builder.WriteString(strconv.Itoa(int(prefix & 0xFF)))
	builder.WriteByte('/')
	builder.WriteString(strconv.Itoa(int(depth)))

	result := builder.String()
	pools.ReturnBuilderToPool(builderPool, builder)
	return result
}
